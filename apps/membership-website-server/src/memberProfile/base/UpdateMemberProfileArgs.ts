/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MemberProfileWhereUniqueInput } from "./MemberProfileWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { MemberProfileUpdateInput } from "./MemberProfileUpdateInput";

@ArgsType()
class UpdateMemberProfileArgs {
  @ApiProperty({
    required: true,
    type: () => MemberProfileWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => MemberProfileWhereUniqueInput)
  @Field(() => MemberProfileWhereUniqueInput, { nullable: false })
  where!: MemberProfileWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => MemberProfileUpdateInput,
  })
  @ValidateNested()
  @Type(() => MemberProfileUpdateInput)
  @Field(() => MemberProfileUpdateInput, { nullable: false })
  data!: MemberProfileUpdateInput;
}

export { UpdateMemberProfileArgs as UpdateMemberProfileArgs };
